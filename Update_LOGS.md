...
- **Permanent Scoped Auto-Cloning:** Made the SitRep auto-cloning feature permanent in `AddReport.jsx` by rendering it as an always-enabled static card while preserving the source report selection dropdown.
- **Province-Scoped Auto-Clone Detection:** Refactored the backend query in `POST /api/situational-reports` to restrict auto-clone source detection by province, preventing a new report for one province from automatically cloning data from a different province's report.
